...
namespace CustomMethodBenjamin.Pages
{
    public partial class ProductsComponent : ComponentBase
    {
        ...
        IEnumerable<CustomMethodBenjamin.Models.ConData.Product> _getProductsResult;
        protected IEnumerable<CustomMethodBenjamin.Models.ConData.Product> getProductsResult
        {
            get
            {
        ...
        protected async System.Threading.Tasks.Task Splitbutton0Click(RadzenSplitButtonItem args)
        {
            if (args?.Value == "csv")
            {
                await ConData.ExportProductsToCSV(new Query() { Filter = $@"{grid0.Query.Filter}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "ProductCategory", Select = "ProductID,ProductName,ProductCategory.CategoryName as ProductCategoryCategoryName,UnitPrice,Picture" }, $"Products");

            }

            if (args == null || args.Value == "xlsx")
            {
                await ConData.ExportProductsToExcel(new Query() { Filter = $@"{grid0.Query.Filter}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "ProductCategory", Select = "ProductID,ProductName,ProductCategory.CategoryName as ProductCategoryCategoryName,UnitPrice,Picture" }, $"Products");

            }
        }

        protected async System.Threading.Tasks.Task Grid0RowSelect(CustomMethodBenjamin.Models.ConData.Product args)
        {
